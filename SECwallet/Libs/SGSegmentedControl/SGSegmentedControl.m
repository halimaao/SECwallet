...
/** 点击按钮时, 指示器的动画移动时间 */
static CGFloat const indicatorViewTimeOfAnimation = 0.3;
...
- (instancetype)initWithFrame:(CGRect)frame delegate:(id<SGSegmentedControlDelegate>)delegate segmentedControlType:(SGSegmentedControlType)segmentedControlType titleArr:(NSArray *)titleArr btn_Margin:(CGFloat)btn_Margin{
...
+ (instancetype)segmentedControlWithFrame:(CGRect)frame delegate:(id<SGSegmentedControlDelegate>)delegate segmentedControlType:(SGSegmentedControlType)segmentedControlType titleArr:(NSArray *)titleArr btn_Margin:(int)btn_Margin {
    return [[self alloc] initWithFrame:frame delegate:delegate segmentedControlType:segmentedControlType titleArr:titleArr btn_Margin:btn_Margin];
}
...
- (void)setupTitleArr {
    ...
    if (self.segmentedControlType == SGSegmentedControlTypeScroll) {
        ...
        for (NSUInteger i = 0; i < _title_Arr.count; i++) {
            ...
            _title_btn.titleLabel.font = BoldSystemFontOfSize(btn_fondOfSize);
            _title_btn.tag = i;
            
            // 计算内容的Size
            CGSize buttonSize = [self sizeWithText:_title_Arr[i] font:BoldSystemFontOfSize(btn_fondOfSize) maxSize:CGSizeMake(MAXFLOAT, button_H)];
            
            // 计算内容的宽度
            CGFloat button_W = 2 * _btn_Margin + buttonSize.width;
            ...
            [_title_btn setTitleColor:TEXT_DARK_COLOR forState:(UIControlStateNormal)];
        ...
        // 计算scrollView的宽度
        CGFloat scrollViewWidth = CGRectGetMaxX(self.subviews.lastObject.frame) +Size(45);
        ...
        // 指示器默认在第一个选中位置
        // 计算TitleLabel内容的Size
        CGSize buttonSize = [self sizeWithText:firstButton.titleLabel.text font:BoldSystemFontOfSize(btn_fondOfSize) maxSize:CGSizeMake(MAXFLOAT, self.frame.size.height)];
...
- (void)setupImageAndTitleArr {
    ...
    if (self.segmentedControlType == SGSegmentedControlTypeScroll) {
        ...
        for (NSUInteger i = 0; i < _title_Arr.count; i++) {
            ...
            _image_title_btn.titleLabel.font = BoldSystemFontOfSize(btn_fondOfSize);
            ...
            [_image_title_btn setTitleColor:TEXT_DARK_COLOR forState:(UIControlStateNormal)];
    ...
    if (self.segmentedControlType == SGSegmentedControlTypeScroll) {
        ...
        if (self.segmentedControlIndicatorType == SGSegmentedControlIndicatorTypeCenter) {
            [UIView animateWithDuration:0.20 animations:^{
                self.indicatorView.SG_width = button.SG_width - _btn_Margin;
            ...
            [UIView animateWithDuration:0.20 animations:^{
                self.indicatorView.SG_width = button.SG_width - 2 * _btn_Margin;
                self.indicatorView.SG_centerX = button.SG_centerX;
            }];
        }
        ...
        if (self.segmentedControlIndicatorType == SGSegmentedControlIndicatorTypeCenter) {
            ...
            [UIView animateWithDuration:indicatorViewTimeOfAnimation animations:^{
                // 计算内容的Size
                CGSize buttonSize = [self sizeWithText:button.titleLabel.text font:BoldSystemFontOfSize(btn_fondOfSize) maxSize:CGSizeMake(MAXFLOAT, self.frame.size.height - indicatorViewHeight)];
                self.indicatorView.SG_width = buttonSize.width + _btn_Margin;
                self.indicatorView.SG_centerX = button.SG_centerX;
            }];
...
- (void)setSegmentedControlIndicatorType:(SGSegmentedControlIndicatorType)segmentedControlIndicatorType {
    ...
    if (self.segmentedControlIndicatorType == SGSegmentedControlIndicatorTypeBottom) {
        ...
        _indicatorView.SG_width = buttonSize.width + _btn_Margin;
        _indicatorView.SG_centerX = firstButton.SG_centerX;

        CGFloat indicatorViewHeight = Size(18);
        ...
        self.indicatorView.layer.cornerRadius = Size(9);
        ...
        if (self.segmentedControlType == SGSegmentedControlTypeScroll) {
            ...
            _indicatorView.SG_width = buttonSize.width + 2 * _btn_Margin;
        } else {
        ...
        if (self.segmentedControlType == SGSegmentedControlTypeScroll) {
            ...
            _bgIndicatorView.SG_width = buttonSize.width + 2 * _btn_Margin;
        } else {
